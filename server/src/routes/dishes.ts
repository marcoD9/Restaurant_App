...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const newDish = await createDish(name, description, price, image);
...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const dish = await updateDishById(id, {
        name,
        description,
        price,
        image,
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const dish = await deleteDishById(id);
      if (dish) {
        res
          .status(200)
          .send({ message: `Dish with id ${id} successfully deleted!`, dish });
      } else {
